...
1. GitHub Pages 静态 demo：展示产品界面、样本账单效果、Agent Assist workflow。
...
- `docs/demo/` 面试和 sales 讲稿。
- `docs/validation/` 验证矩阵。
- 自动化测试文件。
- README。
...
1. 把项目推到 GitHub。
2. 确认 `app/dist` 里的静态 demo 已经由 synthetic data 构建。
...
“为了隐私，我把在线 demo 的账号、电话号码、账单号全部换成 synthetic data。公开网页只展示产品 workflow；真实 PDF 解析能力留在本地 server 和测试里展示。这样既能保护客户信息，也能让面试官看到完整产品思路。”
